...
#INFO: Investigat the discretization effects by looking at the two momentum values with the same Q^2
import sys
...
def readfitdata(folder,error):
    try:
        ...
        error=True
    return dA0,dE0,tlist,chisq,error
...
def ratioplotter(ratiolist, unpertlist, pars, opnum, best_tmin_oneexp, fold, plots=True):
    """
    plot the effective FF
    """
    ...
    tmin1 = best_tmin_oneexp[1][3]
    tmin2 = best_tmin_oneexp[1][4]
    ...
    folderenergy = pars.basedir1+mom[:8]+'fit_data_'+str(pars.nboot)+'/energyfit_'+pars.operators[opnum]+mom[:7]+'_Aexp' + pars.snkfold[pars.sink]+'_tmin'+str(tmin1)+'.csv'
    error = False
    A0,E0,tlist,chisq,error = readfitdata(folderenergy,error)
    energyfit = [ A0, E0]
    ...
    # for quark in np.arange(1,3):
        ...
        # folderenergyshift=pars.basedir1+mom[:8]+'fit_data_'+str(pars.nboot)+'/energyshiftfit_'+pars.operators[opnum]+'_q'+str(quark)+mom[:7]+'_Aexp' + pars.snkfold[pars.sink]+'_tmin'+str(tmin)+'.csv'
        # error = False
        # dA0,dE0,tlist,chisq,error = readfitdata(folderenergyshift,error)
        # energyshiftfit = [dA0,dE0]
        # tlist = np.array([int(i) for i in tlist])
        # chisq = float(chisq[0])
    ...
    for quark in np.arange(2):
        time = np.arange(0,len(ratiolist[0][quark]))
        xlim = 18
        efftime = time[:-1]+0.5

        folderenergyshift=pars.basedir1+mom[:8]+'fit_data_'+str(pars.nboot)+'/energyshiftfit_'+pars.operators[opnum]+'_q'+str(quark+1)+mom[:7]+'_Aexp' + pars.snkfold[pars.sink]+'_tmin'+str(tmin1)+'.csv'
        error = False
        dA0,dE0,tlist,chisq,error = readfitdata(folderenergyshift,error)
        energyshiftfit = [dA0,dE0]
        tlist = np.array([int(i) for i in tlist])
        chisq = float(chisq[0])
        fitBS = np.array([ stats.effmass(fitfunction.eval(time[:xlim+1], [fd.values[nbo] for fd in energyshiftfit])) for nbo in range(energyshiftfit[0].nboot) ])
        fitBSavg   = np.average(fitBS,axis=0)
        fitBSstd   = fitBS.std(axis=0, ddof=1)
        fitBSlower = fitBSavg - fitBSstd
        fitBSupper = fitBSavg + fitBSstd

        folderenergyshift2=pars.basedir1+mom[:8]+'fit_data_'+str(pars.nboot)+'/energyshiftfit_'+pars.operators[opnum]+'_q'+str(quark+1)+mom[:7]+'_Aexp' + pars.snkfold[pars.sink]+'_tmin'+str(tmin2)+'.csv'
        error = False
        dA0,dE0,tlist2,chisq2,error = readfitdata(folderenergyshift2,error)
        energyshiftfit = [dA0,dE0]
        tlist2 = np.array([int(i) for i in tlist2])
        chisq2 = float(chisq2[0])
        fitBS2 = np.array([ stats.effmass(fitfunction.eval(time[:xlim+1], [fd.values[nbo] for fd in energyshiftfit])) for nbo in range(energyshiftfit[0].nboot) ])
        ...
        ratioavg   = np.array([fd.Avg for fd in energyshiftfit])
        ratiostd   = np.array([fd.Std for fd in energyshiftfit])
        
        ydata1 = ratiolist[0][quark]
        yavgeff1 = np.array([y.Avg for y in stats.effectivemass(ydata1)])
        yerreff1 = np.array([y.Std for y in stats.effectivemass(ydata1)])
        ydata2 = ratiolist[1][quark]
        yavgeff2 = np.array([y.Avg for y in stats.effectivemass(ydata2)])
        yerreff2 = np.array([y.Std for y in stats.effectivemass(ydata2)])

        pypl.figure(figsize=(7,6))
        pypl.errorbar(efftime[:xlim], yavgeff1[:xlim], yerreff1[:xlim], fmt='.', capsize=4, elinewidth=1, color='k', marker=pars.markers[pars.sink], markerfacecolor='none'
                      , label=r'$q='+np.array2string(pars.qval[3], separator=', ')+'$')
        pypl.errorbar(efftime[:xlim], yavgeff2[:xlim], yerreff2[:xlim], fmt='.', capsize=4, elinewidth=1, color='b', marker=pars.markers[pars.sink], markerfacecolor='none'
                      , label=r'$q='+np.array2string(pars.qval[4], separator=', ')+'$')

        pypl.plot(tlist[:-1]+0.5,fitBSavg[tlist[:-1]], linestyle='-', color='k', label=r"$\chi^2_{{ \textrm{{dof}} }} ={:0.2f}$".format(chisq))
        pypl.fill_between(efftime[:xlim], fitBSlower, fitBSupper, color='k', alpha=0.2, linewidth=0)
        pypl.plot(tlist2[:-1]+0.5,fitBSavg2[tlist2[:-1]], linestyle='-', color='b', label=r"$\chi^2_{{ \textrm{{dof}} }} ={:0.2f}$".format(chisq2))
        pypl.fill_between(efftime[:xlim], fitBSlower2, fitBSupper2, color='b', alpha=0.2, linewidth=0)
        
        pypl.legend(fontsize='small')
        pypl.xlabel(r'$\textrm{t/a}$',labelpad=14,fontsize=18)
        op = pars.operators[opnum]
        pypl.ylabel(r'$a\Delta E_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}$',labelpad=5,fontsize=18)
        # pypl.title(title)
        # pypl.ylim(ylim)
        pypl.axhline(0, color='k')
        pypl.ylim(-1e-5, 2e-5)

        pypl.savefig(fold + 'Eff_dEshift_q'+np.array2string(pars.qval[pars.momentum],separator='')[1:-1] + '_quark'+str(quark+1)+['_g2','_g4'][opnum]+'.pdf')
        # pypl.show()
        pypl.close()
    # pypl.cla()


def ratioplotter2(ratiolist, unpertlist, pars, opnum, best_tmin_oneexp, fold, plots=True):
    """
    plot the effective FF
    """
    print('\n',pars.operators[opnum])
    ...
    for quark in np.arange(2):
        ...
        ydata1 = ratiolist[0][quark]
        ydata2 = ratiolist[1][quark]
        ydata = [ydata1[i]*ydata2[i]**(-1) for i in range(len(ydata1))]
        for i in range(len(ydata)):
            ydata[i].Stats()
        yavg1 = np.array([y.Avg for y in ydata])
        yerr1 = np.array([y.Std for y in ydata])

        pypl.figure(figsize=(7,6))
        pypl.errorbar(time[:xlim], yavg1[:xlim], yerr1[:xlim], fmt='.', capsize=4, elinewidth=1, color='k', marker=pars.markers[pars.sink], markerfacecolor='none', label='ratio of q=(4,4,2) and q=(6,0,0)')
        pypl.legend(fontsize='small')
        pypl.xlabel(r'$\textrm{t/a}$',labelpad=14,fontsize=18)
        op = pars.operators[opnum]
        pypl.ylabel(r'$R^{q=(4,4,2)}_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}/ R^{q=(6,0,0)}_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}$',labelpad=5,fontsize=18)
        pypl.axhline(1, color='k', lw=0.5, alpha=0.4)
        pypl.ylim(0.99999, 1.00007)
        pypl.savefig(fold + 'dEshift_ratioratio_q'+np.array2string(pars.qval[3],separator='')[1:-1] + 'q'+np.array2string(pars.qval[4],separator='')[1:-1] + '_quark'+str(quark+1)+['_g2','_g4'][opnum]+'.pdf')
        pypl.close()

        # Effective mass of the ratio of ratios
        yavgeff1 = np.array([y.Avg for y in stats.effectivemass(ydata)])
        yerreff1 = np.array([y.Std for y in stats.effectivemass(ydata)])

        pypl.figure(figsize=(7,6))
        pypl.errorbar(time[:xlim], yavgeff1[:xlim], yerreff1[:xlim], fmt='.', capsize=4, elinewidth=1, color='k', marker=pars.markers[pars.sink], markerfacecolor='none', label='ratio of q=(4,4,2) and q=(6,0,0)')
        # pypl.legend(fontsize='small')
        pypl.xlabel(r'$\textrm{t/a}$',labelpad=14,fontsize=18)
        op = pars.operators[opnum]
        # pypl.ylabel(r'$a\Delta E_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}$',labelpad=5,fontsize=18)
        pypl.ylabel(r'$\textrm{Effmass}[R^{q=(4,4,2)}_{'+[r'u',r'd'][quark]+', \gamma_{'+op[-1]+'}}/ R^{q=(6,0,0)}_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}]$',labelpad=5,fontsize=18)
        pypl.axhline(0, color='k', lw=0.5, alpha=0.4)
        pypl.ylim(-3e-5, 2e-5)
        pypl.savefig(fold + 'Eff_dEshift_ratioratio_q'+np.array2string(pars.qval[3],separator='')[1:-1] + 'q'+np.array2string(pars.qval[4],separator='')[1:-1] + 'quark'+str(quark+1)  + '.pdf')
        # pypl.show()
        pypl.close()


        # Difference of the ratios
        ydatadiff = [(ydata1[i]-ydata2[i])*pars.almb**(-1) for i in range(len(ydata1))]
        for i in range(len(ydatadiff)):
            ydatadiff[i].Stats()
        yavgdiff = np.array([y.Avg for y in ydatadiff])
        yerrdiff = np.array([y.Std for y in ydatadiff])
        pypl.figure(figsize=(7,6))
        pypl.errorbar(time[:xlim], yavgdiff[:xlim], yerrdiff[:xlim], fmt='.', capsize=4, elinewidth=1, color='k', marker=pars.markers[pars.sink], markerfacecolor='none', label='difference between q=(4,4,2) and q=(6,0,0)')
        pypl.legend(fontsize='small')
        pypl.xlabel(r'$\textrm{t/a}$',labelpad=14,fontsize=18)
        op = pars.operators[opnum]
        pypl.ylabel(r'$a\Delta E_{'+['u','d'][quark]+', \gamma_{'+op[-1]+'}}/\lambda$',labelpad=5,fontsize=18)
        pypl.axhline(0, color='k', lw=0.5, alpha=0.4)
        pypl.ylim(-1, 1)
        pypl.savefig(fold + 'dEshift_diff_q'+np.array2string(pars.qval[pars.momentum],separator='')[1:-1] + 'quark'+str(quark+1)  + '.pdf')
        pypl.close()
...
if __name__ == "__main__":
    ...
    plotfolder = '/home/mischa/Documents/PhD/analysis_results/feynhellfit2/b5p65kp122005kp122005c2p4800/discretization/'
    ...
    for j in [3,4]:
        ...
        unpertlist.append(unpert_2p)

    ratioplotter(ratiolist, unpertlist, pars, opnum, best_tmin_oneexp, plotfolder, plots=True)
    ratioplotter2(ratiolist, unpertlist, pars, opnum, best_tmin_oneexp, plotfolder, plots=True)
