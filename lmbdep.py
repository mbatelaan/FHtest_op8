...
#INFO: Investigat the dependence of the results on the value of lambda
import sys
...
def weighted_avg(pars, op=1,exclude0=False):
    ...
    for j,mom in enumerate(pars.momfold):
        try:
            ...
            pass
    return np.array(means), np.array(mean_errs), best_tmin_ratio, best_tmin_oneexp

def weighted_avg1(pars, op=1,exclude0=False):
    if exclude0: minmom=1
    else: minmom=0
    #INFO: For the data from the ratiofit first
    tminlist   = []
...
if __name__ == "__main__":
    ...
    # Get the weighted average for lambda
    parslp001 = pr.params(18, 1)
    parslp001.fit = 'FFplot'
    parslp001.makeresultdir(plots=True)
    means, mean_errs = weighted_avg1(parslp001, op=opchoice,exclude0=False)

    parslp0001 = pr.params(7, 1)
    parslp0001.fit = 'FFplot'
    parslp0001.makeresultdir(plots=True)
    means2, mean_errs2 = weighted_avg1(parslp0001, op=opchoice,exclude0=False)

    plotfolder = '/home/mischa/Documents/PhD/analysis_results/feynhellfit2/b5p65kp122005kp122005c2p4800/lambda_dependence/'
    if not os.path.isdir(plotfolder):
        os.makedirs(plotfolder)

    nboot = means[0][1].nboot

    quarknum = 2 #1=up, 2=down
    for mom in [0, -1]:
        print(f"{means2[mom][quarknum].Avg=}")
        print(f"{means2[mom][quarknum].Std=}")
        
        means2[mom][quarknum] = means2[mom][quarknum]*-1
        means2[mom][quarknum].Stats()
        # means[mom][quarknum] = means[mom][quarknum]*-1
        # means[mom][quarknum].Stats()
        yaverages = np.array([means2[mom][quarknum].Avg, means[mom][quarknum].Avg])
        yerrors = np.array([means2[mom][quarknum].Std, means[mom][quarknum].Std])
        lambdas = np.array([-1e-4, 1e-3])
        cminv   = np.diag(yerrors**(-2))
        # cminv = stats.Invcovmat(yvalues, fullcov=True)
        fitres = []
        p0 = 1
        for boot in range(nboot):
            yvalues = np.array([means2[mom][quarknum].values[boot], means[mom][quarknum].values[boot]])
            res=syopt.minimize(ff.chisqfn,p0,args=(linear,lambdas,yvalues,cminv),method='L-BFGS-B',bounds=None,options={'disp': False})
            fitres.append(res.x)
        print(f"{np.average(fitres)=}")
        lambdasfull = np.array([-1e-4, 0, 1e-3])
        fitrange = np.array([linear(lambdasfull, fitres[i]) for i in range(nboot)])
        fitavg = np.average(fitrange, axis=0)
        fitstd = np.std(fitrange, axis=0, ddof=1)
        fitlower = fitavg - fitstd
        fitupper = fitavg + fitstd
        print(f"{fitupper=}")
        
        pypl.figure("lambdadep", figsize=(9,6))
        pypl.errorbar(lambdas, yaverages, yerrors, fmt='.', capsize=4, elinewidth=1, color='k', marker=markers[0], markerfacecolor='none', label=r'$Q^2=0$')
        pypl.plot(lambdasfull, fitavg, color=colors[0], linewidth=0.5)
        pypl.fill_between(lambdasfull, fitlower, fitupper, color=colors[0], alpha=0.5, linewidth=0)
        pypl.xlabel(r'$a\lambda$',labelpad=14,fontsize=18)
        pypl.ylabel(r'$a\Delta E_{u}$',labelpad=5,fontsize=18)
        pypl.axhline(0, alpha=0.4)
        pypl.axvline(0, alpha=0.4)
        ax = pypl.gca()
        pypl.subplots_adjust(bottom=0.17, top=.91, left=0.16, right=0.93)
        pypl.savefig(plotfolder + ['lambdadep_q000.pdf', 'lambdadep_q632.pdf'][mom])
        pypl.close()

    # yaverages1 = np.array([0, means[-1][2].Avg, means2[-1][2].Avg])
    # yerrors1 = np.array([0, means[-1][2].Std, means2[-1][2].Std])
    # lambdas = np.array([0, 1e-3, 1e-4])
    # cminv1   = np.diag(yerrors1**(-2))
    # cminv1[0][0] = 1e20
    # # cminv = stats.Invcovmat(yvalues, fullcov=True)
    # fitres1 = []
    # p0 = 1
    # for boot in range(nboot):
    #     yvalues = np.array([0, means[0][1].values[boot], means2[0][1].values[boot]])
    #     res=syopt.minimize(ff.chisqfn,p0,args=(linear,lambdas,yvalues,cminv),method='L-BFGS-B',bounds=None,options={'disp': False})
    #     fitres1.append(res.x)
    # print(f"{np.average(fitres1)=}")
    # fitrange = np.array([linear(lambdas, fitres1[i]) for i in range(nboot)])
    # fitavg = np.average(fitrange, axis=0)
    # fitstd = np.std(fitrange, axis=0, ddof=1)
    # fitlower = fitavg - fitstd
    # fitupper = fitavg + fitstd

    # pypl.figure("lambdadep2", figsize=(9,6))
    # pypl.errorbar(lambdas, yaverages, yerrors, fmt='.', capsize=4, elinewidth=1, color='k', marker=markers[0], markerfacecolor='none', label=r'$Q^2=0$')
    # pypl.plot(lambdas, fitavg, color=colors[0], linewidth=0.5)
    # pypl.fill_between(lambdas, fitlower, fitupper, color=colors[0], alpha=0.5, linewidth=0)
    # pypl.xlabel(r'$a\lambda$',labelpad=14,fontsize=18)
    # pypl.ylabel(r'$a\Delta E_{u}$',labelpad=5,fontsize=18)
    # # pypl.title(r'lambda dependence')
    # pypl.grid(True, alpha=0.4)
    # ax = pypl.gca()
    # pypl.subplots_adjust(bottom=0.17, top=.91, left=0.16, right=0.93)
    # pypl.savefig(plotfolder + 'lambdadep_q632.pdf')
    # pypl.close()

    
    # best_tmin_ratioM, best_tmin_oneexpM = weighted_avg(parslp001, op=0,exclude0=True)

    # for j,mom in enumerate(parslp001.momfold):
    #     #parslp001ratio = pr.params(kappas, sinktype, j)
    #     parslp001.momentum = j
    #     #Set the operator choice correctly for the given momentum
    #     if j==0: parslp001.numbers=2; parslp001.opchoice=[1] #For Q=0, only calculate the electric FF.
    #     else: parslp001.numbers=10; parslp001.opchoice=[0,1] #For Q=0, only calculate the electric FF.
    ...
    #     nucleon_data = read_data(parslp001)
    #     # Loop over the two operator types
    #     for opnum in parslp001.opchoice:
    #         ratios, unpert_2p = makeratio(nucleon_data,opnum)
    #         # If kappa=kp119930kp119930 then also read the gamma3 files
    #         if parslp001.kappas==14 and parslp001.momentum!=0:
    #             nucleon_datag3 = read_datag3_ratio(parslp001,sgn=-1.0)
    #             ratiosg3, unpert_2pg3 = makeratiog3(nucleon_datag3)
    #             ratios, unpert_2p = combine_ratios(ratios, ratiosg3, unpert_2p, unpert_2pg3)
    #         print('yas-s')

    #         ratioplotter(ratios, unpert_2p, fitfnctwoexp, fitfncratio, parslp001, opnum, best_tmin_ratio, best_tmin_oneexp, plots=True)
